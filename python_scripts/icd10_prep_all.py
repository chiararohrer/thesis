...
import random
import yaml
from preprocessing_functions import find_disease, find_sr_disease, find_chapter
...
disease_names = [f'{l}{i}{j}' for i in range(0,10) for l in ['A', 'B', 'C','D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L', 'M', 'N', 'O', 'P', 'Q'] for j in range(0,10)]
for disease in disease_names:
    #if not os.path.isfile(f'/mypath/diseases/icd10_filtered/filtered_{disease}.tsv'):
    if icd_df[['f.eid']+ code_list].applymap(lambda x: disease in str(x)).sum().sum() > 1000:
        
        print(disease)
        returned_df = find_chapter(icd_df[['f.eid']+ code_list + date_list], disease)
        if returned_df.shape[0]>1000:
            returned_df.to_csv(f'/mypath/diseases/icd10_filtered/filtered_{disease}.tsv', sep='\t', index=False, header=True)
            print(f'{disease} was stored with n_cases: {returned_df.shape[0]}', flush=True)
