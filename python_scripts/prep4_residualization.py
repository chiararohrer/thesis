...
def residualize(path, config_name, size, run=None):
    ...
    #interim_path = f'{path}/interim_data_{size}'
    ...
    encoded_pqtl_df, pqtl_fn, pqtl_map, pqtl_shape = load_pt(interim_path, f'pQTL_{size}')
    ...
    features_cont = pd.concat([encoded_urine_df, encoded_bb_df, encoded_bc_df, encoded_metabolomics_df, encoded_physical_df, encoded_proteomics_df, encoded_inf_df, encoded_gen_df, encoded_pqtl_df], axis=1)
    ...
    write_pt(interim_res_path, f'pQTL_{size}', encoded_pqtl_df, residuals, pqtl_fn, pqtl_map, False, pqtl_shape)
