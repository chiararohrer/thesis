...
def run_all_print(folder, result_folder, interim_folder, id_file, output_folder, upper_cutoff=3650, letter=None, store_models=True, seed=None):
    ...
    model_names = ['Condition']+['train_incidence', 'test_incidence', 'train_samples', 'test_samples']
    ...
    for disease in disease_names:
        ...
        try:
            train_incidence, test_incidence, train_samples, test_samples = predict_evaluate_print(disease, folder, result_folder, interim_folder, id_file, output_folder, upper_cutoff, letter, store_models, seed)
            row = [disease, train_incidence, test_incidence, train_samples, test_samples]
            result_matrix = result_matrix._append(pd.Series(row, index=model_names), ignore_index=True) 
        except: print('FAILURE IN: ', {disease})
...
def predict_evaluate_print(disease_name, folder, result_folder, interim_folder, id_file, output_folder, upper_cutoff, letter, store_models=True, seed=None):
    os.makedirs(output_folder, exist_ok=True)
    output_split = output_folder.split('/')[-1]
    ...
    try:
        ...
        interim_full_train, interim_full_test, merged_cols, train_id, test_id = load_interim_new(interim_path, id_path, letter)
    except:
        print('ecxeption')
        id_path = f'/mypath/{folder}/data_T/{id_file}.txt'
        interim_full_train, interim_full_test, merged_cols, train_id, test_id = load_interim_new(interim_path, id_path, letter, True)
    ...
    ### DROP PATIENTS WITH NANS:
    mask_train, mask_test = (interim_full_train.iloc[:, 1:] != 0).any(axis=1), (interim_full_test.iloc[:, 1:] != 0).any(axis=1)
    interim_full_train, interim_full_test = interim_full_train[mask_train], interim_full_test[mask_test]
    ...
    # keep only those latent patients which also are in interim data
    latent_full_train, latent_full_test = latent_full_train[latent_full_train['f.eid'].isin(interim_full_train['f.eid'].tolist())], latent_full_test[latent_full_test['f.eid'].isin(interim_full_test['f.eid'].tolist())]
    ...
    print('LATENT DATA: ', latent_full_train.shape, latent_full_test.shape)
    print('FULL (INTERIM) DATA: ', interim_full_train.shape, interim_full_test.shape)
    ...
    return latent_full_train['binary'].sum(), latent_full_test['binary'].sum(), latent_full_train.shape[0], latent_full_test.shape[0]
...
def predict_evaluate(disease_name, folder, result_folder, interim_folder, id_file, output_folder, upper_cutoff, letter, store_models=True, seed=None):
    ...
    print('FULL_BLOCK: ', block_full, flush=True)
    ...
    print('----- MODEL B: CoxPH on lasso-selected features -----')
    lasso = LogisticRegression(penalty='l1', C=0.1, solver='saga', random_state=seed, tol=0.01, max_iter=50)
    X_train = interim_full_train.iloc[:, 1:interim_dim+1]
    E_train = interim_full_train['binary']
    
    print('lasso inputs: ', X_train, E_train, E_train.sum())
    start_time = time.time()
    ...
    print('lasso test: ', X_test, E_test)
    ...
    print('----- MODEL C: XGBoost with CoxPH on all features -----')
    if block_full==False:
    ...
    print('----- MODEL E: XGBoost with CoxPH on lasso-selected features -----')
    y_train, y_test = [r['delta_days'] if r['binary']==1 else -r['delta_days'] for i, r in interim_full_train.iterrows()], [r['delta_days'] if r['binary']==1 else -r['delta_days'] for i, r in interim_full_test.iterrows()]
    ...
    print('----- MODEL F: XGBoost with AFT on interim -----')
    if block_full==False:
    ...
    print('----- MODEL H: XGBoost with AFT on lasso-selected features -----')
    y_upper_train, y_upper_test = np.array([+np.inf if r['binary']==0 else r['delta_days'] for i, r in interim_full_train.iterrows()]), np.array([+np.inf if r['binary']==0 else r['delta_days'] for i, r in interim_full_test.iterrows()])
    y_lower_train, y_lower_test = np.array([r['delta_days'] for i, r in interim_full_train.iterrows()]), np.array([r['delta_days'] for i, r in interim_full_test.iterrows()])
    x_train, x_test = interim_full_train[selected_cols[:-2]].to_numpy(), interim_full_test[selected_cols[:-2]].to_numpy()
    aft_train = xgboost.DMatrix(x_train)
...
def load_interim_new(interim_path, id_path, single_name, split=True):
...
def prepare_latent(disease_name, folder, result_folder, id_file, upper_cutoff, letter):
    ...
    if id_file == 'id_67307' or id_file == 'id_52026':
        colnames = [f"reduced_dim{i}" for i in range(2)]
        colnames.extend([f"dim{i}" for i in range(128)])
        latent = pd.read_csv(f'/mypath/{folder}/{result_folder}/latent_space.csv', header=None, names=colnames)
    else:
        latent = pd.read_csv(f'/mypath/{folder}/{result_folder}/latent_space.csv')

    try:
        ids = pd.read_csv(f'/mypath/{folder}/data_{letter}/{id_file}.txt', sep=' ', header=None, names=['f.eid'])
    except:
        ids = pd.read_csv(f'/mypath/{folder}/data_T/{id_file}.txt', sep=' ', header=None, names=['f.eid'])
...
if __name__ == "__main__":
    ...
    if disease_name == 'all':
        run_all_print(folder, result_folder, interim_folder, id_file, output_folder, upper_cutoff, letter, store_models, seed)
