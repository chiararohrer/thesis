...
def prep(folder_name, sample_num, reduce_proteomics, letter, seed=None):
    ...
    '''
    phenotypes_path = '/projects/ukbiobank-AUDIT/data/phenotypes/cur/ukb677323.tab.gz'
    olink_path = '/projects/ukbiobank-AUDIT/data/olink/2024_04_10-ukb-ppp_55k/olink_data.txt.gz'
    ...
    withdrawn_ids = withdrawal['f.eid'].tolist()
    not_related = pd.read_csv('/mypath/patients/with_snp.tsv', sep='\t')
    all_ids = not_related['f.eid'].tolist()

    id400k = pd.read_csv('/mypath/prep3/data_A/id_400000.txt', header=None)
    id400k = id400k[id400k.columns[0]].to_list()

    sampled_ids = list(set(all_ids) - set(id400k))
    sampled_ids = list(set(sampled_ids) - set(withdrawn_ids))
